...
func TestParse(t *testing.T) {
	...
	tests := []struct {
		...
		{"Valid UUID", args{uuidString}, UUID{uuid.UUID(uuidBytes)}, false},
		{"Invalid UUID", args{""}, UUID{}, true},
	}
...
func TestUUID_Size(t *testing.T) {
	...
	tests := []struct {
		...
		{"Valid UUID", fields{&UUID{uuid.UUID(uuidBytes)}}, 16},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := u.Size(); got != tt.want {
				t.Errorf("UUID.Size() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestUUID_MarshalTo(t *testing.T) {
	...
	tests := []struct {
		...
		{"Valid UUID", fields{&UUID{uuid.UUID(uuidBytes)}}, args{make([]byte, 0)}, 16, false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("UUID.MarshalTo() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("UUID.MarshalTo() = %v, want %v", got, tt.want)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := u.Unmarshal(tt.args.data); (err != nil) != tt.wantErr {
				t.Errorf("UUID.Unmarshal() error = %v, wantErr %v", err, tt.wantErr)
			}
			if u.UUID != tt.want {
				t.Errorf("UUID.Unmarshal() = %v, want %v", u.UUID, tt.want)
			}
		})
	}
}
...
func TestUUID_MarshalBSONValue(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("UUID.MarshalBSONValue() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("UUID.MarshalBSONValue() got = %v, want %v", got, tt.want)
			}
			if !reflect.DeepEqual(got1, tt.want1) {
				t.Errorf("UUID.MarshalBSONValue() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
